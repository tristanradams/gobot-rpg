...
public partial class Player : FightingCharacter
{
	...
	protected override string[] AttackAnimations =>
		[CommonFightingCharacterAnimation.Attack, PlayerAnimation.Attack2, PlayerAnimation.Attack3];
	...
	private void UpdateSprite(float direction)
	{
		...
		if (!IsOnFloor())
		{
			...
			{
				CurrentState = State.Jumping;
				Sprite.Play(CommonFightingCharacterAnimation.Jump);
			}
			else
			{
		...
		else if (_isRunning)
		{
			...
			Sprite.Play(CommonCharacterAnimation.Run);
		}
		else
		{
			CurrentState = State.Walking;
			Sprite.Play(CommonCharacterAnimation.Walk);
		}
	}
